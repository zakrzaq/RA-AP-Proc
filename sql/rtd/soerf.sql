SELECT
    ...
    CASE
        WHEN user = 'DLFERGUS' THEN 'dlferguson'
        WHEN user = 'MVANWEE' THEN 'MVANWEE'
        WHEN user = 'JZAKRZE' THEN 'JAKUB.ZAKRZEWSKI'
        ELSE 'OES-DQM'
    END AS user_email,
    ...
    CASE
        ...
        AND mara.mtpos_mara LIKE 'ZRS%' THEN 'Y'
        ELSE 'N'
...
WHERE
    mvke.mandt (+) = '400'
    AND mara.mandt = '400'
    AND mara.matnr = decode(
        translate(soerf.matnr, '-0123456789', '-'),
        '',
        lpad(soerf.matnr, 18, '0'),
        soerf.matnr
    )
    ...
    AND upper(TRIM(soerf.service)) IN (
        'SALES ORG EXTENSION',
