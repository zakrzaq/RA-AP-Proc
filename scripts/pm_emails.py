def pm_emails(server=False):
    ...
    if active_wt_pce_req.empty:
        output += output_msg(server, 'NO CCC REQUESTS')
    else:
    ...
    if active_wt_gts_req.empty:
        output += output_msg(server, 'NO GTS REQUESTS')
    else:
        ...
        output += output_msg(server, f'GTS requests: {len(active_wt_gts_req)}')
        # GTS REQUEST FILE
    ...
    if active_wt_local_req.empty:
        output += output_msg(server, 'NO LOCALIZATION REQUESTS')
    else:
