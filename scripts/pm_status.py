def pm_status():
    ...
    need_gts = ((selected_active_view['target sorg'].isin(['5008'])) & (selected_active_view['INDIA GST\nINHTS'].isna()) & (selected_active_view['INDIA GST\nmarc.stuec'].isna()) & (
        selected_active_view['status'].str.contains('cancel|complete|hold', case=False) == False) & (selected_active_view['status'].str.contains('GST data needed;') == False))
    ...
    # MATNRs LOCAL NEEDED
    need_local = ((selected_active_view['MTART/GenItemCat'].isin(['ZFG', 'ZTG', 'ZNFG', 'ZRS1', 'ZRS4'])) & (~selected_active_view['target plant'].isin(['5070'])) & (selected_active_view['target plant mrp type'].isin(
        ['NA'])) & (selected_active_view['status'].str.contains('cancel|complete|hold', case=False) == False) & (selected_active_view['status'].str.contains('Localization required;') == False))
